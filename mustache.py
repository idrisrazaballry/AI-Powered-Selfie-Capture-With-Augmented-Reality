...
face_mesh_videos = mp_face_mesh.FaceMesh(
    ...
    max_num_faces=1, # Changed from 2 for consistency with main.py and simplicity
...
def overlay(image, filter_img, face_landmarks):
    ...
    if filter_img.shape[2] < 4:
        print("Warning: Mustache image is not a 4-channel transparent PNG. Skipping.")
        return image
    ...
    # Determine new size for the filter (e.g., 2.0x the width of the mouth)
    target_width = int(w_mouth * 2.0)
    ...
    # Center the mustache slightly below the nose (midpoint between nose tip and upper lip)
    ...
    # Resize the mustache filter to the *original* target size
...
def apply_mustache(frame):
    """Applies the mustache filter to a single frame (expected by main.py)."""
    ...
    if mustache is None:
        print(f"❌ Mustache file not found. Returning original frame.")
        return frame
